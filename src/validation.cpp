...
#if defined(NDEBUG)
#error "EVO_Nodes cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uevo) are considered zero fee (for relaying, mining and transaction creation)
...
static bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransactionRef& _tx, bool fLimitFree, bool* pfMissingInputs, int64_t nAcceptTime, bool fOverrideMempoolLimit, bool fRejectAbsurdFee, bool ignoreFees, std::vector<COutPoint>& coins_to_uncache) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    AssertLockHeld(cs_main);
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k EVO
    ...
    if (nHeight > 0 && nHeight < nLast) {
        return 2000 * COIN;
    } else if (nHeight >= nLast && nHeight <= 20000) {
        return 3 * COIN;
    } else if (nHeight > 20000 && nHeight <= 70000) {
        return 5 * COIN;
    } else if (nHeight > 70000 && nHeight <= 110000) {
        return 8 * COIN;
    } else if (nHeight > 110000 && nHeight <= 150000) {
        return 11 * COIN;
    } else if (nHeight > 150000 && nHeight <= 200000) {
        return 15 * COIN;
    } else if (nHeight > 200000 && nHeight <= 230000) {
        return 18 * COIN;
    } else if (nHeight > 230000 && nHeight <= 260000) {
        return 25 * COIN;
    } else if (nHeight > 260000 && nHeight <= 300000) {
        return 30 * COIN;
...
{
    util::ThreadRename("evo_nodes-scriptch");
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // EVO_Nodes Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t)consensus.nEVO_NodesBadBlockTime) &&
            (block.nBits == (uint32_t)consensus.nEVO_NodesBadBlockBits)) {
            // accept EVO_Nodes block minted with incorrect proof of work threshold
            return true;
        }
