...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error(" EVO_NodesMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error(" EVO_NodesMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf(" EVO_NodesMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("evo_nodes-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running  EVO_NodesMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf(" EVO_NodesMiner exception");
    } catch (...) {
        LogPrintf(" EVO_NodesMiner exception");
    }

    LogPrintf(" EVO_NodesMiner exiting\n");
}
