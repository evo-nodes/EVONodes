 ...
 */
const char * const  EVO_Nodes_PID_FILENAME = "evo_nodes.pid";

fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid",  EVO_Nodes_PID_FILENAME));
...
void Shutdown()
{
    ...
    util::ThreadRename("evo_nodes-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)",  EVO_Nodes_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)",  EVO_Nodes_PID_FILENAME));
#endif
...
{
    util::ThreadRename("evo_nodes-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that EVO_Nodes is running in a usable environment with all
...
{
    // Make sure only a single EVO_Nodes process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf",  EVO_Nodes_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if EVO_Nodes is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if EVO_Nodes is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // EVO_Nodes: load previous sessions sporks if we have them.
