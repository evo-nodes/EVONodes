...
/**
 * Fees smaller than this (in uevo) are considered zero fee (for transaction creation)
                                      ...
                                      std::string& strError)
{
    ...
    if (txOut.nValue != consensus.GetMNCollateral(chainHeight)) {
        strError = strprintf("Invalid collateral tx value, must be %s EVO", FormatMoney(Params().GetConsensus().GetMNCollateral(chainHeight)));
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-evo_nodes-address
...
bool CWallet::CreateCoinstakeOuts(const CEvoStake& stakeInput, std::vector<CTxOut>& vout, CAmount nTotal) const
{
        ...
        bool stopOnNewBlock) const
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CEvoStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
...
CWallet* CWallet::CreateWalletFromFile(const std::string& name, const fs::path& path)
{
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_EVO_Nodes && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_EVO_Nodes);
        }
