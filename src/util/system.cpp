...
const char * const  EVO_Nodes_CONF_FILENAME = "evonodes.conf";
const char * const  EVO_Nodes_MASTERNODE_CONF_FILENAME = "masternode.conf";


// EVO_Nodes only features
// Masternode
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "evo_nodesd -foo=bar
...
#else
    const char* pszModule = "evo_nodes";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\EVO_Nodes
// Windows >= Vista: C:\Users\Username\AppData\Roaming\EVO_Nodes
// Mac: ~/Library/Application Support/EVO_Nodes
// Unix: ~/.evo_nodes
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EVONodes";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EVONodesParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EVONodesParams
    // Mac: ~/Library/Application Support/EVONodesParams
    // Unix: ~/.evo_nodes-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EVONodesParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == nullptr || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "EVONodesParams";
#else
    // Unix
    return pathRet / ".evonodes-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/evo_nodes/sapling-spend.params";
        sapling_output = "/usr/share/evo_nodes/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/evo_nodes/sapling-spend.params";
            sapling_output = "/usr/local/share/evo_nodes/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf",  EVO_Nodes_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()) {
        // Create empty evonodes.conf if it does not exist
