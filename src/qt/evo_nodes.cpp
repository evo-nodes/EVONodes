...
#include "qt/evo_nodes/splash.h"
#include "qt/evo_nodes/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/evo_nodes/governancemodel.h"
...
{
    return QCoreApplication::translate("evo_nodes-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in evo_nodes.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in evo_nodes.qrc)
...
/** Class encapsulating EVO_Nodes Core startup and shutdown.
...
/** Main EVO_Nodes application object */
...
public:
    ...
    /// Get window identifier of QMainWindow ( EVO_NodesGUI)
...
{
    window = new  EVO_NodesGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, & EVO_NodesGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, & EVO_NodesGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet( EVO_NodesGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet( EVO_NodesGUI::DEFAULT_WALLET);
        ...
        // EVO_Nodes: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, & EVO_NodesGUI::handlePaymentRequest);
        connect(window, & EVO_NodesGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. EVO_Nodes can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(evo_nodes_locale);
    Q_INIT_RESOURCE(evo_nodes);
    ...
    /// 6. Determine availability of data directory and parse evonodes.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf",  EVO_Nodes_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // evo_nodes: links repeatedly have their payment requests routed to this process:
