    ...
    case Mode::Encrypt: {
        ...
        if (ret) {
            newpassCache = newpass1;
             EVO_NodesGUI* window = static_cast< EVO_NodesGUI*>(parentWidget());
...
{
     EVO_NodesGUI* gui = static_cast< EVO_NodesGUI*>(parentWidget());
...
{
    hide();
    static_cast< EVO_NodesGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
