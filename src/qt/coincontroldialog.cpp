...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select EVO Outputs to Spend" :
            "Select Shielded EVO to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::EVO;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show EVO_Nodes address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
