...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::EVO);
    ...
    if (settings.contains("nAnonymize EVO_NodesAmount") || reset)
        gArgs.SoftSetArg("-anonymizeevo_nodesamount", settings.value("nAnonymize EVO_NodesAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in evo_nodes.cpp during shutdown to also remove the window positions
