...
#include "qt/evo_nodes/topbar.h"
#include "askpassphrasedialog.h"
#include "qt/evo_nodes/forms/ui_topbar.h"
#include "qt/evo_nodes/loadingdialog.h"
#include "qt/evo_nodes/lockunlock.h"
#include "qt/evo_nodes/qtutils.h"
#include "qt/evo_nodes/receivedialog.h"

#include "addresstablemodel.h"
#include "bitcoinunits.h"
#include "clientmodel.h"
#include "optionsmodel.h"
#include "qt/evo_nodes/balancebubble.h"
...
TopBar::TopBar(EVO_NodesGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                             ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitle3, ui->labelTitle4, ui->labelTitle5, ui->labelTitle6,
        ui->labelTitle8, ui->labelMNReward};
    ...
    setCssProperty({ui->labelAmountEvo}, "amount-topbar");
    setCssProperty({ui->labelPendingEvo, ui->labelImmatureEvo, ui->labelAvailableEvo, ui->labelLockedEvo,
                       ui->labelCollateralEvo, ui->labelMNRewardvalue},
        "amount-small-topbar");
    ...
    ui->pushButtonConf->setButtonText("evonodes.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open evonodes.conf with default application"));
}
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("EVO_Nodes")
    updateDisplayUnit();
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    if (g_tiertwo_sync_state.IsSynced()) {
        ui->labelCollateralEvo->setText(GUIUtil::formatBalance(CMasternode::GetMNCollateral(chainActive.Tip()->nHeight) * COIN, nDisplayUnit));

        CAmount Blockvalue = GetBlockValue(chainActive.Tip()->nHeight);
        CAmount MNReward = Blockvalue;
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    if (walletModel) {
        nLockedBalance = walletModel->getLockedBalance(true);
    }

    CAmount nAvailableBalance = newBalance.balance - nLockedBalance;

    // EVO Total
    QString totalEvo = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // Expanded
    ui->labelAmountEvo->setText(GUIUtil::formatBalance(newBalance.balance + newBalance.immature_balance, nDisplayUnit));
    ui->labelAvailableEvo->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    ui->labelPendingEvo->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureEvo->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
    ui->labelLockedEvo->setText(GUIUtil::formatBalance(nLockedBalance, nDisplayUnit));
}
