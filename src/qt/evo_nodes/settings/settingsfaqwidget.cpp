...
#include "config/evo_nodes-config.h"
#endif

#include "qt/evo_nodes/settings/settingsfaqwidget.h"
#include "qt/evo_nodes/settings/forms/ui_settingsfaqwidget.h"
#include "qt/evo_nodes/mnmodel.h"
#include "qt/evo_nodes/qtutils.h"

#include <QScrollBar>
#include <QMetaObject>

SettingsFaqWidget::SettingsFaqWidget( EVO_NodesGUI* parent, MNModel* mnModel) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableEVO,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableEVO,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("EVO Nodes is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. EVO Nodes incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("EVO Nodes utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of EVO Nodes to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableEVOContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received EVO Nodes requires 6 confirmations on the network "
               "to become eligible for spending which can take 6-10 minutes.")) +
        formatFAQParagraph(
            tr("Your EVO Nodes wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableEVO->setText(unspendableEVOContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of EVO Nodes with a minimum of 60 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("evo_nodes-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ui->labelContent_Stake->setText(stakeContent);

    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("For support, please visit our website: %1")
                .arg("<a style='color: #C8FF87' href='https://evonodes.io/'>" + tr("https://evonodes.io") + "</a>")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(mnModel->getMNCollateralRequiredAmount(), BitcoinUnits::EVO)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in EVO Nodes Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the EVO Nodes network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableEVO, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableEVO);});
    ...
    if (parent)
        connect(parent, & EVO_NodesGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
