...
#include "qt/evo_nodes/forms/ui_snackbar.h"
#include "qt/evo_nodes/qtutils.h"
#include <QTimer>


SnackBar::SnackBar( EVO_NodesGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, & EVO_NodesGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
