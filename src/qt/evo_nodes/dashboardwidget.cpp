...
#include "qt/evo_nodes/dashboardwidget.h"
#include "qt/evo_nodes/forms/ui_dashboardwidget.h"
#include "qt/evo_nodes/sendconfirmdialog.h"
#include "qt/evo_nodes/txrow.h"
...
DashboardWidget::DashboardWidget( EVO_NodesGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    //setCssProperty(ui->labelSquareEvo, "square-chart-evo");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    // setCssProperty(ui->labelEvo, "text-chart-evo");
    ...
    //setCssProperty(ui->labelAmountEvo, "text-stake-evo-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, & EVO_NodesGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(16,20,10);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isEvo = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair EVO, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            evo = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalEvo += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesEvo.append(evo);
        chartData->valuesMN.append(mn);

        int max = std::max(evo, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(117,181,42));
    set1->setColor(QColor(117,181,42));
    ...
    set0->append(chartData->valuesEvo);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalEvo > 0 || chartData->totalMN > 0) {
        //setCssProperty(ui->labelAmountEvo, "text-stake-evo");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        //setCssProperty(ui->labelAmountEvo, "text-stake-evo-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({/*ui->labelAmountEvo,*/ ui->labelAmountMN});
    //ui->labelAmountEvo->setText(GUIUtil::formatBalance(chartData->totalEvo, nDisplayUnit));
