...
#include "qt/evo_nodes/addnewcontactdialog.h"
#include "qt/evo_nodes/forms/ui_send.h"
#include "qt/evo_nodes/guitransactionsutils.h"
#include "qt/evo_nodes/loadingdialog.h"
#include "qt/evo_nodes/optionbutton.h"
#include "qt/evo_nodes/qtutils.h"
#include "qt/evo_nodes/sendchangeaddressdialog.h"
#include "qt/evo_nodes/sendconfirmdialog.h"
...
SendWidget::SendWidget( EVO_NodesGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a EVO Nodes URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-evo");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onEVOSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onEVOSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent EVOs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield EVOs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent EVOs to shield."));
    }
}
...
void SendWidget::onEVOSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto evoAdd = Standard::DecodeDestination(address.toStdString(), isStaking, isExchange, isShielded);

        if (!Standard::IsValidDestination(evoAdd) || isStaking) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(evoAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(evoAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
