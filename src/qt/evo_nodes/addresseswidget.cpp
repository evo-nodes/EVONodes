...
#include "qt/evo_nodes/forms/ui_addresseswidget.h"
#include "qt/evo_nodes/addressfilterproxymodel.h"
#include "qt/evo_nodes/addresslabelrow.h"
#include "qt/evo_nodes/tooltipmenu.h"
...
AddressesWidget::AddressesWidget( EVO_NodesGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto evoAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStaking, isExchange, isShield);

        if (!Standard::IsValidDestination(evoAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(evoAdd);
        if (walletModel->updateAddressBookLabels(evoAdd, label.toUtf8().constData(),
