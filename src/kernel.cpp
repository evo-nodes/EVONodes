...
#include "staking_address.json.h"
...
static bool LoadStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nHeight)
{
    ...
    stake = std::unique_ptr<CStakeInput>(CEvoStake::NewEvoStake(txin, nHeight, block.nTime));
...
bool CheckProofOfStake(const CBlock& block, std::string& strError, const CBlockIndex* pindexPrev)
{
    ...
    UniValue v = read_json(LoadValidStakingAddresses());
    ...
    for (unsigned int idx = 0; idx < v.size(); idx++) {
        ...
        const UniValue& vScriptPubkey = find_value(o, "scriptpubkey");
        if (!vScriptPubkey.isStr())
            continue;

        if (HexStr(stakePrevout.scriptPubKey) == vScriptPubkey.get_str()) {
            matchFound = true;
            break;
        } 
    }
