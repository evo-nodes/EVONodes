...
The build script we'll be using is contained in the  EVO_Nodes github repository ([contrib/gitian-build.py](https://github.com/evo_nodes-project/evo_nodes/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the  EVO_Nodes repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/ EVO_Nodes-Project/ EVO_Nodes/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, evo_nodes-detached-sigs, and evo_nodes GitHub repos.
