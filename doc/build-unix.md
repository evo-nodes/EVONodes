UNIX BUILD NOTES
====================
Some notes on how to build EVO_Nodes Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile EVO_Nodes Core and the dependencies,
...
This will build evo_nodes-qt as well, if the dependencies are met.
...
memory available when compiling EVO_Nodes Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~evo_nodes/+archive/berkeley-db4).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:evo_nodes/berkeley-db4
    sudo apt-get update
...
To build EVO_Nodes Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build evo_nodes-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a evo_nodes-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip evo_nodesd" to strip the debug
...
Security
--------
To help make your EVO_Nodes Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, EVO_Nodes Core should be built with a non-executable stack
