 EVO_Nodes Core version *v5.0.0* is now available from:  <https://github.com/evo_nodes-project/evo_nodes/releases>
...
==============
 ...
 EVO_Nodes Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Overview
Users will be able to protect their financial information by sending and receiving EVOs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://evo_nodes.org .
...
* Top bar showing shield EVO balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield EVO balance.
...
See the release notes from [v4.2.0](https://github.com/ EVO_Nodes-Project/ EVO_Nodes/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed EVO supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in EVO
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in EVO
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zEVOsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield EVO)
  The `moneysupply` JSON return entry is the total EVO supply (the sum of all transparent and shield EVO)
...
| `getaccountaddress` | Returned the current  EVO_Nodes address associated with an internal account | Account system removal |
| `setaccount` | Associate a  EVO_Nodes address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given  EVO_Nodes address | Account system removal |
| `getaddressesbyaccount` | Returned a list of  EVO_Nodes addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a  EVO_Nodes address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a  EVO_Nodes address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zEVO balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zEVO mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zEVO mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zEVO amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zEVO to a EVO address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zEVO mints to a EVO address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zEVOs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzevoseed` | Set the wallet's deterministic zEVO seed to a specific value | Zerocoin deprecation |
| `getzevoseed` | Return the current deterministic zEVO seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zEVO seed | Zerocoin deprecation |
| `dzevostate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzevo` | Do a search for deterministically generated zEVO that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_evo_nodes unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake evo_nodesd and unit tests: fixing missing MacOS framework. (furszy)
