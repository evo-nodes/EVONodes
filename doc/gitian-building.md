...
*Setup instructions for a gitian build of  EVO_Nodes Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the  EVO_Nodes
...
to the  EVO_Nodes GitHub release page.
...
- [Building  EVO_Nodes Core](#building-evo_nodes-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/evo_nodes-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/evo_nodes-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/evo_nodes-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/evo_nodes-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/evo_nodes-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/evo_nodes-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/evo_nodes-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@evo_nodes.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/evo_nodes-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/evo_nodes-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/evo_nodes-osx-*-build.assert
```
