...
- [Development guidelines](#development-guidelines)
    - [General  EVO_Nodes Core](#general-evo_nodes-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
------------------------------------------
 ...
 EVO_Nodes Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play EVOs (tEVO)" on the test network, if you
...
### DEBUG_LOCKORDER
 ...
 EVO_Nodes Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/evo_nodes-project/evo_nodes/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_evo_nodes
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_evo_nodes --log_level=test_suite
$ valgrind -v --leak-check=full src/evo_nodesd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_evo_nodes.coverage/index.html`.
```

**Sanitizers**
 EVO_Nodes can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates EVOs (if wallet is enabled).
...
However, in open source software such as  EVO_Nodes Core, where everyone uses
...
pay attention to for reviewers of  EVO_Nodes Core code.

General  EVO_Nodes Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*:  EVO_Nodes Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against  EVO_Nodes Core so that they can be integrated
...
- src/chiabls
  - Upstream at https://github.com/ EVO_Nodes-Project/bls-signatures/ ; actively maintained by Core contributors, deviates from upstream https://github.com/Chia-Network/bls-signatures/
...
file descriptors it will cause problems with  EVO_Nodes's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com: EVO_Nodes-Project/ EVO_Nodes.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `evo_nodes-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `evo_nodes-cli` and the GUI debug console use this table to determine how to
