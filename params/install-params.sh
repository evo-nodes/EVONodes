...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/EVONodesParams"
    else
        PARAMS_DIR="$HOME/.evonodes-params"
    fi
fi
...
function install_params {
    ...
    if ! [ -f "$filename" ]
    then
        filename="share/evo_nodes/$filename"
    fi
...
function main() {
    ...
    cat <<EOF
EVO_Nodes - install-params.sh

This script will install the EVO_Nodes zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common EVO_Nodes zkSNARK parameters. Note that it is
