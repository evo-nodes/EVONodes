...
def remove_current_translations():
    ...
    This leaves evo_nodes_en.ts untouched.
    """
...
def update_build_systems():
    ...
    """
    filename_lang = [re.match(r'((evo_nodes_(.*)).ts)$', filename).groups() for (filename, filepath) in all_ts_files(include_source=True)]
    filename_lang.sort(key=lambda x: x[0])

    # update qrc locales
    with open('src/qt/evo_nodes_locale.qrc', 'w', encoding="utf8") as f:
