...
class ReorgStakeTest( EVO_NodesTestFramework):
    ...
    def check_money_supply(self, expected_evo):
        # verify that nodes have the expected EVO supply
        evo_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(evo_supply, [DecimalAmt(expected_evo)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that EVO supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check EVO supply...")
