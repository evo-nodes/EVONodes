...
`test/functional/test_framework/test_framework.py: EVO_NodesTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/evo_nodes_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_evo_nodesd = config["components"].getboolean("ENABLE_BITCOIND")
    ...
    if not (enable_wallet and enable_utils and enable_evo_nodesd):
        print("No functional tests to run. Wallet, utils, and evo_nodesd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, keep_cache="rewrite"):
    # Warn if evo_nodesd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "evo_nodesd"]) is not None:
            print("%sWARNING!%s There is already a evo_nodesd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/evo_nodesd' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/src/evo_nodes-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie evo_nodesds, we can apply a
...
class RPCCoverage():
    ...
    commands per `evo_nodes-cli help` (`rpc_interface.txt`).
