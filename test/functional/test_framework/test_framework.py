...
TMPDIR_PREFIX = "evo_nodes_func_test_"


class  EVO_NodesTestFramework():
    """Base class for a evo_nodes test script.

    Individual evo_nodes test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave evo_nodesds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop evo_nodesds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing evo_nodesd/evo_nodes-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use evo_nodes-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: evo_nodesds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a evo_nodesd test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple evo_nodesd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as evo_nodesd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in evonodes.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in evonodes.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "evo_nodesd"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run evo_nodesds:
    ...
    #  EVO_Nodes Specific TestFramework
    ...
    """
    Create a ProReg tx, which references an 100 EVO UTXO as collateral.
...
'''
 EVO_NodesTestFramework extensions
'''
...
class  EVO_NodesDMNTestFramework( EVO_NodesTestFramework):
...
# !TODO: remove after obsoleting legacy system
class  EVO_NodesTier2TestFramework( EVO_NodesTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [["-nuparams=v5_shield:249", "-nuparams= EVO_Nodes_v5.5:250", "-nuparams=v6_evo:250", "-whitelist=127.0.0.1"]] * self.num_nodes
